...
public class JabutiGUI extends JFrame {
	...
	//Mujava Button
	private JMenu mujavaMenu = new JMenu();
	
	private JMenuItem mujavaGenMutants = new JMenuItem();

	private JMenuItem mujavaRunTests = new JMenuItem();
	...
	private JMenuItem testCaseReduction = new JMenuItem();
...
//		menuBar.add(qualipsoMenu);
	...
	/**
	 * MUJAVA This method is responsible to build the Mujava Tool into Jabuti
	 * and to add it to the Menu Bar
	 */
	private void buildMujavaMenu() {
		mujavaMenu.setText("Mujava");
		
		mujavaGenMutants.setText("Gerar Mutantes");

		mujavaRunTests.setText("Executar Testes");

		mujavaGenMutants.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				mujavaGenMutants_actionPerformed(e);
			}
		});
		
		mujavaRunTests.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				mujavaRunTests_actionPerformed(e);
			}
		});
		
		mujavaMenu.add(mujavaGenMutants);
		mujavaMenu.add(mujavaRunTests);
		menuBar.add(mujavaMenu);
	}
	
	// mujavaGenMutants action performed
	   void mujavaGenMutants_actionPerformed(ActionEvent ex) {
			TableSorter.setSortedColumn(-1); /*
												 * Every time when the table is
												 * called from the menu, uses the
												 * default order
		...
		// mujavaGenMutants action performed
		 void mujavaRunTests_actionPerformed(ActionEvent ex) {
				TableSorter.setSortedColumn(-1); /*
													 * Every time when the table is
													 * called from the menu, uses the
													 * default order
