...
import mujava.results.CalculationResultsExecution;
...
public class RunTestPanel extends JPanel implements ActionListener {
	...
	void jbInit() {
		...
		// Summary Tables for traditioanl mutants and class mutants (x,y) =
		// (0,1)
		...
		if ((targetClassObj != null) && (testSetObject != null)) {
			...
			try {
				if (onlyClassButton.isSelected()) {
					...
					System.out.println("Mutantes mortos");
	...
	private void showEmptyResult(JTable table, JList killed_list, JList live_list) {
		...
		resultModel.setValueAt("  " + (new Integer(0)).toString(), 0, 1); // live
																			// mutant
		resultModel.setValueAt("  " + (new Integer(0)).toString(), 1, 1); // killed
																			// mutant
	...
	private void showResult(TestResult tr, JTable table, JList killed_list, JList live_list) {
		int i;
		//CalculationResultsExecution cre = new CalculationResultsExecution();
		// Mutation Score
		if (tr == null)
			System.out.println("-----------");
		// Mutantes mortos
		int killed_num = tr.killed_mutants.size();
		System.out.println("Mutantes Mortos: " + killed_num);
		// Mutantes mortos vivo
		int live_num = tr.live_mutants.size();
		System.out.println("Mutantes Vivos: " + live_num);
		...
		// Show the result on resultTable
		ResultTableModel resultModel = (ResultTableModel) (table.getModel());
		resultModel.setValueAt("  " + (new Integer(live_num)).toString(), 0, 1); // live
																					// mutant
		resultModel.setValueAt("  " + (new Integer(killed_num)).toString(), 1, 1); // killed
																					// mutant
		resultModel.setValueAt("  " + (new Integer(live_num + killed_num)).toString(), 2, 1); // total
		resultModel.setValueAt("  " + mutant_score.toString() + "%", 3, 1); // mutant
																			// score
		...
		// chamada método EDER
		CreateArtificialARFF createArtificialARFF = new CreateArtificialARFF();
		//Map<String, Double> killedPercentag = cre.killedPercentageByOperators(killed_mutants, live_mutants, killed_num, live_num);
		//createArtificialARFF.creatorRelation("Mujava", killedPercentag);
	...
	/*
	 * void updateContents(String methodSignature){ setMutantPath(); File
	 * mutant_dir = new File(getMutantPath()+"/"+methodSignature); String[]
	 * mutants = mutant_dir.list(new MutantDirFilter());
	 * showGeneratedMutantsNum(mutants); mList.setListData(mutants);
	 * mList.repaint(); clearSourceContents(); showOriginal(); }
	 */
	void showGeneratedTraditionalMutantsNum(String[] name) {
		if (name != null) {
			...
			System.out.println("EDER Total : " + total);
...
class ResultTableModel extends AbstractTableModel {
	...
	/**
	 * AbstractTable Implementation ï¿½Ô¼ï¿½
	 */
