...
public class RunTestPanel extends JPanel implements ActionListener  
{
   ...
   private Map<String, Integer> operadorKilled = new HashMap<String, Integer>();
   ...
   void showGeneratedTraditionalMutantsNum(String[] name)
   {
      if (name != null)
      {
         ...
         for (int i=0; i<MutationSystem.tm_operators.length; i++)
         {
            myModel.setValueAt(new Integer(num[i]), i, 1);
            total = total + num[i];
          //Popula Map com total de cada operador morto
			operadorKilled.put(MutationSystem.tm_operators[i], new Integer(num[i]));
         }
         tmTotalLabel.setText("Total : " + total);
         ResultsMutantsExecution.populaTotalPorOperador(operadorKilled);
      }
   }
